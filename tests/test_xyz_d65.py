...
class TestXYZD65InputOutput(util.ColorAsserts, unittest.TestCase):
    ...
    def test_input_raw(self):
        ...
        self.assertColorEqual(Color("xyz-d65", [1, 1, 1]), Color('color(xyz-d65 100% 100% 100%)'))

    def test_color_class(self):
        """Test raw input."""

        self.assertColorEqual(Color(Color("xyz-d65", [1, 1, 1])), Color('color(xyz-d65 100% 100% 100%)'))

    def test_color(self):
        """Test color input/output format."""

        args = {"color": True}
        color = "color(xyz-d65 0.3 1 0.5)"

        self.assertEqual(Color(color).to_string(**args), 'color(xyz-d65 0.3 1 0.5)')

        color = "color(xyz-d65 0.3 1 0.5 / 0.5)"
        self.assertEqual(Color(color).to_string(**args), 'color(xyz-d65 0.3 1 0.5 / 0.5)')

        color = "color(xyz-d65 30% 100% 50% / 50%)"
        self.assertEqual(Color(color).to_string(**args), 'color(xyz-d65 0.3 1 0.5 / 0.5)')
    ...
    def test_no_alpha(self):
        ...
        color = "color(xyz-d65 0.3 1 0.5 / 0.2)"
        xyzd65 = Color(color)
    ...
    def test_force_alpha(self):
        ...
        color = "color(xyz-d65 0.3 1 0.5 / 100%)"
        xyzd65 = Color(color)
    ...
    def test_precision(self):
        ...
        color = 'color(xyz-d65 0.1234567 0.1234567 0.1234567)'
    ...
    def test_fit(self):
        ...
        self.assertEqual(
            Color('color(xyz-d65 2 -1 0)').to_string(),
            'color(xyz-d65 2 -1 0)'
        )

        self.assertEqual(
            Color('color(xyz-d65 2 -1 0)').to_string(fit="clip"),
            'color(xyz-d65 2 -1 0)'
        )

        self.assertEqual(
            Color('color(xyz-d65 2 -1 0)').to_string(fit=False),
            'color(xyz-d65 2 -1 0)'
        )
...
class TestXYZD65Properties(util.ColorAsserts, unittest.TestCase):
    ...
    def test_x(self):
        ...
        c = Color('color(xyz-d65 0.1 0.2 0.3 / 1)')
