...
class TestsAchromatic(util.ColorAsserts, unittest.TestCase):
    ...
    def test_achromatic(self):
        ...
        self.assertEqual(Color('zcam-jmh', [0, 50, 270]).is_achromatic(), False)
        self.assertEqual(Color('zcam-jmh', [NaN, 50, 270]).is_achromatic(), False)
...
class TestZCAMApperanceModel(util.ColorAsserts, unittest.TestCase):
    ...
    def test_Qz_zero_low_colorfulness(self):
        ...
        for a, b in zip(
            zcam_to_xyz(Qz=0, Mz=0, hz=self.COORDS.hz, env=ZCAMJMh.ENV),
            [0, 0, 0]
        ):
            self.assertCompare(a, b, 14)
