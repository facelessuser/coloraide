...
def vienot(color: 'Color', severity: float, transform: Matrix) -> None:
    ...
    coords = cast(Vector, alg.dot(transform, color[:-1], dims=alg.D2_D1))
    if severity < 1:
...
def machado(color: 'Color', severity: float, matrices: Dict[int, Matrix]) -> None:
    ...
    coords = cast(Vector, alg.dot(m1, color[:-1], dims=alg.D2_D1))
    ...
    if severity1 != severity and severity1 < 10:
        ...
        coords2 = cast(Vector, alg.dot(m2, color[:-1], dims=alg.D2_D1))
