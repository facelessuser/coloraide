...
# General matrices
####################
T = alg.matmul(M, alg.divide(RGB_TO_XYZ, 100))
INV_T = alg.inv(T)
...
lw, mw, sw = alg.matmul(T, white[:-1])
lb, mb, sb = alg.matmul(T, blue[:-1])
...
LMS_475 = alg.matmul(M, [1.3287E-01, 1.1284E-01, 9.422E-01])
LMS_575 = alg.matmul(M, [8.4394E-01, 9.1558E-01, 1.9706E-03])
LMS_485 = alg.matmul(M, [5.6985E-02, 1.6987E-01, 5.864E-01])
LMS_660 = alg.matmul(M, [1.6161E-01, 6.1E-02, 1.1906E-05])
...
for axis, name, wings in zip(AXIS, NAMES, WINGS):
    ...
    alg.pprint(alg.matmul(INV_T, m2))
