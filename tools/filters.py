...
def process_image(img, output, name, amount, space):
    ...
    with Image.open(img) as im:
        ...
        for e, i in enumerate(range(im.size[0])):
            for j in range(im.size[1]):
                pixels[i, j] = apply_filter(name, amount, space, pixels[i, j])
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='diagrams', description='Apply filter to an image.')
    ...
    process_image(args.input, args.output, args.filter, args.amount, args.space)
