...
-   **NEW**: Add new `serialize` function that behaves like `to_string` except that it does not apply gamut mapping by
    default.
-   **NEW**: Add new, experimental gamut mapping approach `oklch-cubic`.
...
-   **BREAK**: Update Rec. 2020 to be display referred using the EOTF in BT.1886 with a gamma of 2.4. This matches the
    ...
    have the old behavior. `rec2020-oetf` is not registered by default and must be registered manually, or the
...
-   **BREAK**: Update Rec. 709 to be display referred using the EOTF in BT.1886 with a gamma of 2.4. This matches the
    ...
    registered by default and must be registered manually, or the user can import `coloraide.everything.ColorAll`
